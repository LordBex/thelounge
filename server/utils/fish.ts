...
// S-boxes (truncated for brevity - use the full arrays from your original code)
const INITIAL_S: readonly number[][] = [
	[
...
const createBlowfish = (key: number[]): BlowfishInstance => {
	...
	for (let i = 0; i < 18; i++) {
		...
		for (let j = 0; j < 4; j++) {
			temp = (temp << 8) | paddedKey[(i * 4 + j) % paddedKey.length];
		}
	...
	const f = (x: number): number => {
		return (
			(((S[0][(x >>> 24) & 0xff] + S[1][(x >>> 16) & 0xff]) ^ S[2][(x >>> 8) & 0xff]) +
				S[3][x & 0xff]) >>>
			0
		);
	};

	const encryptPair = (left: number, right: number): [number, number] => {
		for (let i = 0; i < 16; i++) {
			left ^= P[i];
			right ^= f(left);
		...
		[left, right] = [right, left];
		return [(left ^ P[17]) >>> 0, (right ^ P[16]) >>> 0];
	};

	const decryptPair = (left: number, right: number): [number, number] => {
		for (let i = 17; i > 1; i--) {
			left ^= P[i];
			right ^= f(left);
			[left, right] = [right, left];
	...
	for (let i = 0; i < 18; i += 2) {
		[left, right] = encryptPair(left, right);
		P[i] = left;
		P[i + 1] = right;
	}
	...
	return {
		encryptBlock(block: BlowfishBlock): BlowfishBlock {
			const leftWord = (block[0] << 24) | (block[1] << 16) | (block[2] << 8) | block[3];
			const rightWord = (block[4] << 24) | (block[5] << 16) | (block[6] << 8) | block[7];
			const [encLeft, encRight] = encryptPair(leftWord, rightWord);
		...
		decryptBlock(block: BlowfishBlock): BlowfishBlock {
			const leftWord = (block[0] << 24) | (block[1] << 16) | (block[2] << 8) | block[3];
			const rightWord = (block[4] << 24) | (block[5] << 16) | (block[6] << 8) | block[7];
			const [decLeft, decRight] = decryptPair(leftWord, rightWord);
...
const encodeBase64Block = (left: number, right: number): string => {
	...
	// Encode right first (6 chars)
	for (let i = 0; i < 6; i++) {
		result += FISH_BASE64[right & 0x3f];
		right >>>= 6;
	}

	// Then encode left (6 chars)
	for (let i = 0; i < 6; i++) {
		result += FISH_BASE64[left & 0x3f];
		left >>>= 6;
	}
...
const decodeBase64Block = (chunk: string): [number, number] | null => {
	if (chunk.length !== 12) {
		return null;
	}
	...
	// Decode the first 6 chars into right
	for (let i = 0; i < 6; i++) {
		const val = FISH_BASE64.indexOf(chunk[i]);
	...
	// Decode the next 6 chars into left
	for (let i = 0; i < 6; i++) {
		const val = FISH_BASE64.indexOf(chunk[i + 6]);
...
const fishEncryptECB = (plaintext: string, key: string): string => {
	...
	for (let i = 0; i < data.length; i += 8) {
		...
		const encrypted = blowfish.encryptBlock(block);

		const left =
			(encrypted[0] << 24) | (encrypted[1] << 16) | (encrypted[2] << 8) | encrypted[3];
		const right =
			(encrypted[4] << 24) | (encrypted[5] << 16) | (encrypted[6] << 8) | encrypted[7];

		result += encodeBase64Block(left >>> 0, right >>> 0);
	}
...
const fishDecryptECB = (ciphertext: string, key: string): DecryptResult => {
	...
	const remainder = ciphertext.length % 12;

	if (remainder > 0) {
		processText = ciphertext.slice(0, -remainder);
		isPartial = true;
	}

	// Validate base64 characters
	if (!processText.split("").every((char) => FISH_BASE64.includes(char))) {
		return {text: "", status: "error"};
	}
	...
	try {
		for (let i = 0; i < processText.length; i += 12) {
			const chunk = processText.slice(i, i + 12);
			const decoded = decodeBase64Block(chunk);
			...
			const decrypted = blowfish.decryptBlock(block);
